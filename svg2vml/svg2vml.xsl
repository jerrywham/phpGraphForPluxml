...
<!-- Queste variabili rappresentano una dimensione "standard" in pixel della parte di
     ...
     sua dimensioni oppure ? espressa tramite percentuale.
-->
...
<!-- se l'elemento svg ? il primo (e quindi non eredita attributi) e non ha nessun informazione
...
<v:group>
   ...
   <!-- paint non mi interessa perch? i valori di stroke e fill vengono copiati nei figli.
...
<xsl:template match="svg:g">
   ...
   <!-- contiene:
         -1 (non impostare coordsize, gi? impostato in gruppi precedenti)
   ...
   <xsl:variable name="cs">
    ...
    <!-- se ho transform, lo gestisco chiamando l'opportuno template, transform-ric, a
         ...
         Supponiamo di avere un gruppo con scale (in svg), creer? il mio gruppo (in vml) con 
         ...
         creato in precedenze, il quale non ? possibile recuperarlo dal documento svg,
         perch? era rappresentato da una trasformazione di tipo scale e quindi chiamo una
         ...
         precedenti perch? qui a contare ? il rapporto tra w,h e coordsize (che rappresenta
         la scala).
         [...] 
    -->
...
<!-- style viene gestito dagli elementi che ne fanno uso, proprier? di fill e stroke e
     altro. -->
</xsl:template>

<!-- DA TOGLIERE -->
<xsl:template match="*" mode="head">
<!-- cerca tutti gli elementi, se ? presente style crea l'elemento style -->
...
<!-- se c'? (e ci dev'essere) xlink:href, allora cerca l'elemento e mettilo qui -->

<!-- use si pu? riferire o a symbol o a defs (altri riferimenti non sono gestiti) -->
...
<xsl:variable name="nome-el">
    ...
    <!-- se ho gi? trovato symbol non devo gestire nient'altro -->
    <xsl:otherwise>
        <xsl:if test="$cs != '-1'">
                <xsl:attribute name="coordsize">
                    <xsl:value-of select="$cs" />
...
<xsl:if test="@style">
    <!-- l'attributo style verr? gestito oppotunamente dai template di gestione di
            fill e stroke e di altre propriet? i cui valori possono essere contenuti
            in style -->
</xsl:if>
<xsl:if test="@class">
    <!-- class pu? essere usato per associare alcune propriet? definite nell'elemento
...
<!--                name() = 'marker' or name() = 'pattern' or name() = 'view'">-->
        <!-- se c'? viewBox, allora considera quest'attributo altrimenti no! -->
<!--<xsl:if test="@viewBox">-->
        <!-- analizziamo il valore di align -->
     <!--<xsl:choose>-->
         <!--<xsl:when test="contains(@preserveAspectRatio, 'none')"> -->
            <!-- non devo fare niente, ? il comportamento
                 di default di vml -->
        <!--</xsl:when>-->
...
<!--<xsl:when test="name() = 'image'">-->
    <!-- qui la gestione ? un po' differente -->
<!--</xsl:when>-->
<!--<xsl:otherwise>-->
<!--</xsl:otherwise>-->
<!--</xsl:choose>-->
<!--</xsl:when>-->
    ...
    <!-- se non c'? questo attributo bisogna moficare l'aspetto ugualmente perch? i valori
...
<xsl:choose>
    ...
    <!-- non abbiamo viewBox:
        ? come se ci fosse viewBox con gli ultimi due valori impostati
...
<!-- controllo per inserimento coordsize:
        - se c'? un svg precedente con w,h,vb: non fare niente
        - se non c'?: inserisci coordsize: SCHERMO-X,SCHERMO-Y -->
